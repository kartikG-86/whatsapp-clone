...
const ChatSidebar = () => {
    ...
    const {id} = useParams()
    console.log("ID from chatSidebar" , id)
    ...
    <div className={`col-xl-3 col-lg-4 col-md-4 chat-box ${id && window.matchMedia('(max-width: 1040px) ').matches ? 'd-none' : 'd-block'} `} >
        ...
        {sideBarList && sideBarList.map((user, index) => (
            <div onClick={() => changeChat(user)} key={user._id} className='user-chat d-flex flex-row justify-content-between '>
                <div className='d-flex flex-row justify-content-left'>
                    <img onClick={() => showDp(user.user)} src={user.user.imgUrl} alt={user.user.userName} data-bs-toggle="modal" data-bs-target="#show_dp_modal" />
